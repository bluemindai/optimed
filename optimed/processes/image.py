...
def denoise_image(
    ...
    """
    assert mode in ['gaussian', 'bilateral'], 'mode must be either "gaussian" or "bilateral"'
    assert isinstance(img, np.ndarray), 'data must be a NumPy array'
...
def blur_inside_roi(
    ...
    """
    assert isinstance(img, np.ndarray) and isinstance(roi_mask, np.ndarray), 'data must be a NumPy array'
    ...
    if use_bbox:
        ...
        new_mask[bbox[0][0]:bbox[0][1],
                 bbox[1][0]:bbox[1][1],
                 bbox[2][0]:bbox[2][1]] = True
...
def blur_outside_roi(
    ...
    if use_bbox:
        new_mask = np.zeros_like(roi_mask, dtype=bool)
        new_mask[bbox[0][0]:bbox[0][1], bbox[1][0]:bbox[1][1], bbox[2][0]:bbox[2][1]] = True
        mask_to_use = new_mask
    else:
        mask_to_use = roi_mask.astype(bool)
